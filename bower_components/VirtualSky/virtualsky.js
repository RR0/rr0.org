...
//	this.credit = (location.host == "lcogt.net" && location.href.indexOf("/embed") < 0) ? false : true;
		...
		this.projections = {
			...
			"stereo": {
				...
				azel2xy: function (az, el, w, h) {
					...
					return {
						x: (w / 2 + f * k * h * cosel * sinaz),
						y: (h - f * k * h * (cosel1 * sinel - sinel1 * cosel * cosaz)),
						el: el
					}
				},
				atmos: true
			...
			"gnomic": {
				...
				radec2xy: function (ra, dec, coords) {
					...
					return {
						x: (this.wide / 2) - F * cd * Math.sin(dA),
						y: (this.tall / 2) - F * (cd0 * sd - A * sd0),
						el: coords[0] * this.r2d
					}
				},
				draw: function () {
					if (!this.transparent) {
		...
		function br(i) {
			return "border-radius:" + i + ";-moz-border-radius:" + i + ";-webkit-border-radius:" + i + ";"
		}
		...
		if (this.islive) interval = window.setInterval(function (sky) {
			sky.setClock("now")
		}, 1000, this)
...
// Change the active language
	VirtualSky.prototype.changeLanguage = function (code) {
		for (var i = 0; i < this.langs.length; i++) {
			if (this.langs[i].language.code == code) {
...
// Our stars are stored in decimal degrees so we will convert them here
	...
	VirtualSky.prototype.createSky = function () {
		...
		if (this.objects) {
			...
			for (var o = 0; o < ob.length; o++)
				$.ajax({
					dataType: "jsonp",
					url: "https://www.strudel.org.uk/lookUP/json/?name=" + ob[o],
					context: this,
		...
		this.registerKey("a", function () {
			this.toggleAtmosphere()
		}, "atmos")
		this.registerKey("g", function () {
			this.toggleGround()
		}, "ground")
		this.registerKey("h", function () {
			this.cycleProjection()
		}, "projection")
		this.registerKey("i", function () {
			this.toggleNegative()
		}, "neg")
		this.registerKey(",", function () {
			this.toggleEcliptic()
		}, "ec")
		this.registerKey(";", function () {
			this.toggleMeridian()
		}, "meridian")
		this.registerKey("e", function () {
			this.toggleGridlinesEquatorial()
		}, "eq")
		this.registerKey("z", function () {
			this.toggleGridlinesAzimuthal()
		}, "az")
		this.registerKey("m", function () {
			this.toggleGridlinesGalactic()
		}, "gal")
		this.registerKey("M", function () {
			this.toggleGalaxy()
		}, "galaxy")
		this.registerKey("q", function () {
			this.toggleCardinalPoints()
		}, "cardinal")
		...
		this.registerKey("u", function () {
			this.togglePlanetLabels()
		}, "sollabels")
		this.registerKey("p", function () {
			this.togglePlanetHints()
		}, "sol")
		this.registerKey("o", function () {
			this.toggleOrbits()
		}, "orbits")
		this.registerKey("c", function () {
			this.toggleConstellationLines()
		}, "con")
		this.registerKey("v", function () {
			this.toggleConstellationLabels()
		}, "names")
		this.registerKey("b", function () {
			this.toggleConstellationBoundaries()
		}, "conbound")
		this.registerKey("R", function () {
			this.toggleMeteorShowers()
		}, "meteorshowers")
		this.registerKey("1", function () {
			this.toggleHelp()
		})
		this.registerKey("8", function () {
			this.setClock("now")
		}, "reset")
		this.registerKey("j", function () {
			this.spinIt("down")
		}, "slow")
		this.registerKey("k", function () {
			this.spinIt(0)
		}, "stop")
		this.registerKey("l", function () {
			this.spinIt("up")
		}, "fast")
		this.registerKey("-", function () {
			this.setClock(-86400)
		...
		this.registerKey(38, function () {
			this.changeMagnitude(0.25)
		}, "magup") // up
		this.registerKey(40, function () {
			this.changeMagnitude(-0.25)
		}, "magdown") // down
		this.registerKey(63, function () {
			this.toggleHelp()
		})
	...
	VirtualSky.prototype.toggleHelp = function () {
		...
		if ($("." + v + "_dismiss").length > 0) $("." + v + "_dismiss").trigger("click")
			...
			$("." + v + "_help, ." + v + "_bg").on("mouseout", {sky: this}, function (e) {
	...
	VirtualSky.prototype.registerKey = function (charCode, fn, txt) {
		...
		for (c = 0; c < charCode.length; c++) {
			...
			for (i = 0; i < this.keys.length; i++) {
				if (this.keys.charCode == ch && this.keys.altKey == alt) aok = false
			}
			if (aok) {
				this.keys.push({
					"str": str,
	...
	VirtualSky.prototype.selectProjection = function (proj) {
		if (this.projections[proj]) {
			...
			if (this.container) {
				...
				$("." + this.id + "_projection")
					.on("mouseover", {me: this}, function (e) {
						e.data.me.mouseover = true
					})
					.css({
						position: "absolute",
						padding: 0,
					...
					.delay(500).fadeOut(1000, function () {
					$(this).remove()
				})
			}
		}
...
// Uses algorithm defined in Practical Astronomy (4th ed) by Peter Duffet-Smith and Jonathan Zwart
	VirtualSky.prototype.moonPos = function (JD, sun) {
		var d2r, JD, sun, lo, Po, No, i, e, l, Mm, N, C, Ev, sinMo, Ae, A3, Mprimem, Ec, A4, lprime, V, lprimeprime, Nprime,
			lppNp, sinlppNp, y, x, lm, Bm
		d2r = this.d2r
		JD = JD || this.times.JD
...
// Using B = 1900.0 + (JD − 2415020.31352) / 365.242198781 and p73 Practical Astronomy With A Calculator
	...
	VirtualSky.prototype.positionCredit = function () {
		this.container.find("." + this.id + "_credit").css({
			position: "absolute",
			top: parseFloat(this.tall) - 5 - this.fontsize(),
			left: 5
		})
	}
	...
	VirtualSky.prototype.draw = function (proj) {
		...
		if (this.credit) {
			...
			if (d.find("." + this.id + "_credit").length == 0)
				d.append("<div class=\"" + this.id + "_credit\"><a href=\"https://lcogt.net/virtualsky\" target=\"_parent\" title=\"Created by the Las Cumbres Observatory Global Telescope\">" + this.getPhrase("powered") + "</a></div>")
			d.find("." + this.id + "_credit").css({
				padding: 0,
				zIndex: 20,
				display: "block",
				overflow: "hidden",
			...
			d.find("." + this.id + "_credit a").css({
				display: "block",
				width: Math.ceil(metric_credit) + "px",
				height: fontsize + "px",
	...
	VirtualSky.prototype.lightbox = function (lb) {
		...
		function columize() {
			...
			lb.css({
				"left": Math.floor((this.wide - lb.outerWidth()) / 2) + "px",
				top: ((this.tall - lb.height()) / 2) + "px"
			})
		}
		...
		this.container.find(".virtualsky_dismiss").click({lb: lb, bg: bg}, function (e) {
			lb.remove()
			bg.remove()
		})
		bg.click({lb: lb, bg: bg}, function (e) {
			lb.hide()
			bg.hide()
		})
		// Update lightbox when the screen is resized
		$(window).resize({vs: this, fn: columize}, function (e) {
			e.data.fn.call(e.data.vs)
		})
		return this
	}
...
// Inputs: RA (deg), Dec (deg), duration (seconds)
	VirtualSky.prototype.panTo = function (ra, dec, s) {
		...
		this.panning = {
			s: {ra: this.ra_off * this.r2d, dec: this.dc_off * this.r2d},
			e: {ra: ra, dec: dec},
			duration: s,
			start: new Date()
		}
...
// Animation step for the panning
	...
	VirtualSky.prototype.start = function () {
		...
		if (interval !== undefined) clearInterval(interval)
		interval = window.setInterval(function (sky) {
			sky.setClock("now")
		}, 1000, this)
	}
...
// Some useful functions
	function convertTZ(s) {
		...
		var tzs = {
			A: 1, ACDT: 10.5, ACST: 9.5, ADT: -3, AEDT: 11, AEST: 10, AKDT: -8, AKST: -9,
