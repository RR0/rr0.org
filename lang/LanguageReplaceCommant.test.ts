...
describe("LanguageReplaceCommand", () => {
  ...
  test("add english translation", async () => {
    ...
    expect(outputFile.contents).toBe(`<a href="time/1/9/6/8/CondonReport/index.html">English translation</a>`)
  })
  ...
  test("add english translation", async () => {
    ...
    const command = new LanguageReplaceCommand()
    const outputFile = await command.execute(context)
    expect(outputFile.contents).toBe(`<a href="time/1/9/6/8/CondonReport/index_fr.html">Traduction française</a>`)
  })
})
