...
import {intlOptions} from "../../test/TestUtil"

describe("HtAccessToNetlifyReplaceCommand", () => {

  function newContext(contents: string) {
  ...
  test("redirect html file to html file", async () => {
    ...
    const context = newContext(`Redirect /Documents/Articles/Vallee/1990_5ArgumentsContreHET_Vallee_fr.html https://rr0.org/time/1/9/9/0/Vallee_5ArgumentsAgainstTheExtraterrestrialOriginOfUnidentifiedFlyingObjects/index_fr.html`)
  ...
  test("redirect directory to directory", async () => {
    const command = new HtAccessToNetlifyReplaceCommand()
    const context = newContext(`Redirect /science/crypto/ufologie https://rr0.org/science/crypto/ufo`)
    const file = await command.execute(context)
  ...
  describe("redirect directory to directory", () => {
    ...
    test("with trailing slash", async () => {
      const command = new HtAccessToNetlifyReplaceCommand()
      const context = newContext(`Redirect /science/crypto/ufo/analyse/hypotheses/HET/ https://rr0.org/science/crypto/ufo/analyse/hypotheses/intelligence/HET/`)
      const file = await command.execute(context)
