...
describe("SsiVarCommand", () => {
  ...
  test("replaces title var", async () => {
    ...
    const context = testUtil.newContext("org/eu/fr/asso/spepse/projet/Magonia.html", `<!--#set var="title" value="Le projet Magonia" -->`)
  ...
  test("replaces url var", async () => {
    const command = new SsiVarReplaceCommand("url", (match: string, ...args: any[]) => `<meta name="url" content="${args[0]}"/>`)
    const context = testUtil.newContext("org/eu/fr/dn/gendarmerie/index.html", `<!--#set var="url" value="https://www.defense.gouv.fr/gendarmerie/" -->`)
    const file = await command.execute(context)
